...
const string strMessageMagic = "SHAPES Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    int64_t nSubsidy = 50;
     ...
     else if(pindexBest->nHeight+1 <= 20100)
     {
        nSubsidy = 50 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 100100)
     {
        nSubsidy = 25 * COIN;
        return nSubsidy + nFees;
     }
     else if(pindexBest->nHeight+1 <= 300100)
     {
        nSubsidy = 10 * COIN;
        return nSubsidy + nFees;
     }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SHAPES", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Bitcoin goes mainstream: Circle's payments make cryptocurrencies easy.";
        CTransaction txNew;
        txNew.nTime = 1412056091;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x29d7b8f6b8eea1ada2bfb422ddedd5018518297161a6d4adb70128e44bd9f7fc"));
        block.print();
